...
ggplot(elections_historic, aes(x = popular_pct, y = ec_pct,
  ...
  annotate(geom="text", x = .47, y = .9,
           label = "Bush won just 47.9% of the popular vote",
           fontface = "bold",
           color = "red",
           vjust=42) +
  scale_x_continuous(labels = scales::percent) +
...
ggplot(elections_historic, aes(x = popular_pct, y = ec_pct,
  ...
  geom_text_repel(data = subset(elections_historic,
                                year %in% "2000" |
                                winner %in% "George W. Bush")) +
  annotate(geom="text", x = .3, y = .93,
           label = "Despite the Iraq War, George W. Bush \n Performed Better in 2004 than 2000.",
           hjust=0, 
           fontface="italic", 
           color = "red") +
  ...
  labs(x = "Winner's share of popular vote", 
       y = "Winner's share of electoral college vote", 
       title = "Presidential Elections: Popular & Electoral College Margins", 
       subtitle = "1824-2016") # adds a caption
...
ggplot(elections_historic, aes(x = popular_pct, y = ec_pct,
  ...
  annotate(geom="text", x = .3, y = .93,
           label = "Despite the Iraq War, George W. Bush performed better in 2004 than 2000.",
           hjust=0, 
           fontface="italic", 
           color = "red") +
  annotate(geom = "segment", 
